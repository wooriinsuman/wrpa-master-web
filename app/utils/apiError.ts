...
const MESSAGES: Partial<Record<ApiErrorCode, string>> = {
  ...
  // 관리자/사용자 역할은 회사가 있어야 한다(백엔드 needsCompany). 400이지만 어느 필드가
  // 문제인지 화면이 알 수 없어 등재한다 — 역할을 낮추는 순간 회사가 필수가 되는 게 원인이다.
  ...
  // 정지·완전 삭제뿐 아니라 역할 교체로 마지막 SYSTEM에서 시스템 역할을 떼는 경우도 이 코드다.
